...
- Daten: `src/data/exam_data.json`
  - `exams`: 8 Prüfungstermine, 243 Einträge (221 lernbar, 222 mit Lösung); jede `meta` hat `pruefungsteil` (AP1/AP2). Davon **6 AP1** + **2 AP2-Übungsklausuren (KI-generiert: Set 1 mit 21 Fragen / 92 Punkte, Set 2 mit 22 Fragen / 101 Punkte; je 11 Aufgaben, komplementäre Themen)**. **Alle 6 AP1-Prüfungen sind paraphrasiert** (`meta.paraphrasiert:true`, KI-überarbeitet – neu formuliert, nicht mehr wörtlich, s. ADR-007) und haben `schluesselwoerter`+`thema_tags` → im Klausur-Modus mit Auto-Auswertung. Pipeline: `scripts/import/paraphrasen/` (`extrahiere-quelle.mjs` → `_quelle/` [gitignored, verbatim], Agenten schreiben je `<slug>.json` mit **eigen-neu-formulierten** Aufgaben, `apply-paraphrasen.mjs` spielt idempotent ein). **Verbatim-Originale bleiben in der Git-Historie** (Repo privat halten)
  ...
  - Import reproduzierbar via `scripts/import/*.mjs` (AP2-Fragen: `import-ap2-pruefungsfragen.mjs` + `import-ap2-pruefungsfragen-2.mjs`, je eigener Marker/ID-Präfix, idempotent); Migrationen unter `scripts/migrations/`. **AP1-Prüfungen sind noch wörtlich** und müssen vor Veröffentlichung paraphrasiert werden (ADR-007)
...
- Seiten: `Home` (Dashboard), `Lernen` (frictionless „Heute lernen"-Smart-Session, `lib/lernsession.js`; Modi via `?modus=heute|schwaechen|frei` — **Schwächen-Training** priorisiert via `baueSchwaechenSession` die schwachen Themen-Tags aus `berechneStatistik`), **`Lernpfade`** (geführter, fortschrittsbasierter Lernweg – s. u.), `Flashcards`, `Lernzettel`, `Wiederholen` (FSRS-SRS, 4-stufig + Intervall-Vorschau), `Quiz`, `Lückentext` (Cloze – Lücken **automatisch aus den Lernzetteln** via `lib/glossar`, tolerante Prüfung via `lib/cloze`, Renderer `components/ClozeFrage`; alle rein+getestet), `Klausur` (Prüfungssimulation: Freitext + Schlagwort-Check + optional Timer; Ergebnis zählt hoch via `hooks/useCountUp.js`, reduced-motion-bewusst), `Statistik`, `Suche` (Volltext), `Konto` (Login/Registrierung mit Speicher-Transparenz + Passwort-Bestätigung + Einwilligung + **Konto löschen**), **`Info`** (Über & Datenschutz, `/info`)
