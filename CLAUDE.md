...
- Seiten: `Home` (Dashboard), `Lernen` (frictionless „Heute lernen"-Smart-Session, `lib/lernsession.js`; Modi via `?modus=heute|schwaechen|frei` — **Schwächen-Training** priorisiert via `baueSchwaechenSession` die schwachen Themen-Tags aus `berechneStatistik`), **`Lernpfade`** (geführter, fortschrittsbasierter Lernweg – s. u.), `Flashcards`, `Lernzettel`, `Wiederholen` (FSRS-SRS, 4-stufig + Intervall-Vorschau), `Quiz`, `Lückentext` (Cloze – Lücken **automatisch aus den Lernzetteln** via `lib/glossar`, tolerante Prüfung via `lib/cloze`, Renderer `components/ClozeFrage`; alle rein+getestet), `Klausur` (Prüfungssimulation: Freitext + Schlagwort-Check + optional Timer; Ergebnis zählt hoch via `hooks/useCountUp.js`, reduced-motion-bewusst), `Statistik`, `Suche` (Volltext), `Konto` (Login)
